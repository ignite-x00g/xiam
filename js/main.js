...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (joinForm) {
    joinForm.addEventListener('submit', async (e) => { // Made async
      ...
      const workPrefSelect = document.getElementById('work-preference');
      for (const option of workPrefSelect.options) {
        if (option.selected) {
          workPreference.push(option.value);
        }
      }
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', async (e) => { // Made async
      ...
      try {
        ...
        // Actual fetch to Cloudflare worker
        const response = await fetch('https://contact.gabrieloor-cv1.workers.dev/', { // Note: Ensure trailing slash if needed by worker
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(submissionPayload)
        });
