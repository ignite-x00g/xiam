...
window.initializeJoinUsModal = function() {
    ...
    if (!joinForm.dataset.submissionHandlerAttached) { // Avoid double attachment
        joinForm.addEventListener('submit', function(e) {
            ...
            // const formData = new FormData(joinForm); // Gets basic fields like name, email, phone, interest, comment
            // const basicData = Object.fromEntries(formData.entries());
            //
            // const dynamicData = {};
            // joinUsModal.querySelectorAll('.form-section.completed').forEach(section => {
            //     const sectionName = section.dataset.section;
            //     if (sectionName && sectionName !== "Interest") { // Exclude "Interest" if handled as basic field
            //         dynamicData[sectionName] = [];
            //         section.querySelectorAll('.inputs input').forEach(inputElement => { // Renamed 'input' to 'inputElement'
            //             dynamicData[sectionName].push(inputElement.value);
            //         });
            //     }
            // });
            // const fullApplicationData = { ...basicData, dynamicSections: dynamicData };
            // console.log("Join Us Application Data for API:", fullApplicationData);
            // At this point, 'fullApplicationData' would be sent to a Cloudflare Worker.
            // e.g., fetch('/api/submit-application', { method: 'POST', body: JSON.stringify(fullApplicationData) });
